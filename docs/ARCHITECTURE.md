...
> route of `(class, input_as)`) + `current_index` + `end_of_level_queue` + `level`.

Fields (Schema 2): `schema_version`, `level_route` (the route of **this level**:
a branch or sub-sequence — `(class, input_as)` pairs, resolved by the loader),
`current_index` (0-based position of the class processing in `level_route`),
`end_of_level_queue` (queue at the end of this level's sequence), `level`
(branch-depth counter, starts 1), `launcher_class` (class that started the flow;
constant, informational), `task_id` (public), `branch_id` (a composite's branch
slot identity: **autogenerated** as a `uuid.uuid4()` (same stdlib as `task_id`, no parent-derived naming, collision-free by construction)
— to avoid name collisions; assigned by the submit at flow birth (root branch)
and by each composite at fan-out (child branches); **every message of the flow
carries one** (never empty); constant along the branch's sequence, distinct
between branches, preserved through the branch's execution),
`message_type` (execution_request | execution_result), `payload` (the data —
single container for both roles).

- **Who builds it**: the **submit** seeds the flow on a `main` agent in **level
  1** with `end_of_level_queue` = the **final-result queue**, and assigns the
  **root `branch_id`** (every message of the flow carries one, never empty);
  there is no
...
- **Composite (branching)**: on receiving a request the composite does not advance
   while its branches run; it fans out giving each branch its **loader-resolved
   `level_route`** (`(class, input_as)`),
   `current_index = 0`, `end_of_level_queue` = its **gathering queue** (a second
   physical queue, `gather:<agent_id>` — distinct from its class inbox; the two
   inlets are consumed in one two-cycle poll: intake on the inbox, then a
   **collection gated by pending fan-in**, §12.3/§12.4), `level + 1`
   and a fresh autogenerated `branch_id` (unique — no parent-derived naming —,
   distinct across all levels); on fan-in
   the returning result's `branch_id` names the join slot (so a multi-step branch,
   whose returned `level_route` expands to its own stages, still slots under the
   parent-assigned id). It re-keys its shared `input_as` under each branch's first
...
composite:
  └─ type: composite → branches (fan-out to its branches, gather on its own
       gathering queue — a second physical queue, §12.3; build its output_as;
       a single branch = a sequence)
```
...
- **Declaration**: Schema 3 (`available_tools`) maps each tool name to
  ...
  terse `parameters` (`{arg: {input_as}.{key} | literal}` — explicit
  `{input_as}.{key}`, no implicit resolution); it declares its own
  `input_as`/`input_type`/`input_schema` and `output_as`/`output_type`/
   ...
   queue, the resource is shared with no engine-side concurrency cap
   (`semaphore` stays a future scope, §2).
- **Runtime (ToolAgent)**: resolves `tool: <name>` against the node's
  ...
  declare output capacity (Schema 3). The declared contracts are **verified at
  runtime on both edges** by the common runner (superset, only when declared):
  the incoming data must contain the `input_schema` and the built value under
  `output_as` the `output_schema` (strict types; extras irrelevant). It
  completes with an
...
2. The **submit** seeds the flow: `ExecutionRequestMessage` on the `main` class
   ...
   walks its flow.
3. Composite: a single `type: composite` fans out to its `branches` — each
   branch gets its loader-resolved `level_route` (its `(class, input_as)` steps)
   + `current_index=0` + `level+1` + an autogenerated `branch_id`, and its
   results gather in the composite's gathering queue via `end_of_level_queue` —
   AGENT_LIFECYCLE §12.4.
4. Atomic: lingo if linguistic, tool registry if concrete.
5. Each branch returns through its `end_of_level_queue` (the composite's gathering
   queue) **conserving its `branch_id`**; the composite fans in (per-slot join
   keys `<task_id>:<branch_id>` in a dedicated slots scope, the close
   arbitrated by one atomic continuation delete —
   `state:<composite>`), groups the branch results by their autogenerated
   `branch_id` slots (collision-free even for same-`output_as` leaves) and
   builds its own payload via the pattern's **`build_output_as`** (no engine
   default — construction is the pattern's model, §4.10.6); then it
   resumes its level.
...
- `main` = starting agents (entry points); `type: atomic/composite` — for an
  atomic, `kind: tool/linguistic`; for a composite, `branches` (a list of
  branches, each an ordered list of **bare pattern names**; inner ramification
  is a step whose name references a `type: composite` agent) = capabilities.
  Every agent declares the mandatory entry/output contract triples
  (`input_as/input_type/input_schema` and `output_as/output_type/output_schema`);
  `output_schema` → pydantic model at compile time; `output_as` = result
  namespacing; a tool's call is the terse `parameters` (`{input_as}.{key}` /
  literal), and `tool: <name>` references a Schema 3 `available_tools` key
  (Schema 1).
- Load is **fail-fast**: a pattern with an invalid reference fails the whole
  load (LEG-021); the dry-run validator is LEG-071. Once loaded, the catalog is a
  **dependency DAG** — `invalidate(name)` (LEG-070) marks the pattern and, every
  composite that depends on it transitively (through its `branches`), as
  invalid; the catalog reflects the invalid set and never serves, routes or
  resolves as a branch the invalidated patterns — loudly, never masked (rule 9).
